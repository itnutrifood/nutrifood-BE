...
single-use rotating refresh tokens. Admin records are stored in the `admins` table.
`make init` generates unique PostgreSQL and JWT secrets but intentionally leaves
the bootstrap administrator identity and password empty. Configure them only when
you are ready to seed an administrator:

```sh
ADMIN_USERNAME=<your-administrator-email>
ADMIN_PASSWORD=<a-unique-password-of-at-least-14-characters>
# ADMIN_TOKEN_SECRET is generated by `make init`; do not replace it with an example value.
```
...
```json
{
  "username": "<your-administrator-email>",
  "password": "<your-unique-password>"
}
```

Use the returned access token as `Authorization: Bearer <token>` for admin
endpoints. Refresh tokens can be exchanged once at `POST /api/v1/admin/auth/refresh`;
reusing a consumed token revokes its session family.
